...
def reactive_loop():
    while True:
        ...
        if text_input:
            ...
            from activities import check_trigger, is_active
            resp = check_trigger(text_input)
            if resp == "done":
                pass  # Activity cleared, let LLM respond naturally
            elif resp:
                ...
                if is_active():
                    _speak_streaming("Ask me the first question.", initiated_by="user")
                set_status("idle")
                continue
